"""
Cost & learning dashboard for Model Router v1.0.3.
...
<style>
  :root { --bg:#0f1117; --card:#1a1d27; --fg:#e6e8ee; --dim:#8b90a0;
          --green:#3ddc84; --blue:#5b9dff; --amber:#ffb020; }
...
<body>
<h1>🧠 Model Router — Cost & Learning Dashboard</h1>
<div class="sub">auto-refresh every 10s · <a href="/docs" style="color:var(--blue)">API docs</a></div>
...
<section>
  ...
  <table><thead><tr><th>request</th><th>task</th><th>model</th><th>mode</th><th>preset</th></tr></thead>
...
async function refresh(){
  try {
    const learn = await j('/admin/learning');
    const preset = await j('/admin/preset');
    ...
    document.getElementById('recent').innerHTML = (learn.recent_requests || []).map(r =>
      ...
      (r.routing_mode || '') + '</span></td><td>' + (r.preset || '') + '</td></tr>')
      .reverse().join('') ||
      '<tr><td colspan="5" style="color:var(--dim)">No requests yet</td></tr>';
...
async def dashboard() -> HTMLResponse:
    """Self-contained cost & learning dashboard (answers: how much did I save?)."""
