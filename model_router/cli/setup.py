...
def _select_from_list(
    ...
    if multi:
        raw = _input_colored(
            "Select numbers (e.g., 1,3,5 or 'all' or 'none')",
            C.YELLOW,
        )
        if raw.lower() == "all":
        ...
        raw = _input_colored("Select number", C.YELLOW)
        try:
            idx = int(raw) - 1
            if 0 <= idx < len(options):
...
def _print_banner() -> None:
    ...
    print(f"{C.GREEN}{C.BOLD}  Model Router — Interactive Setup Wizard{C.RESET}")
    print(f"{C.GREEN}{C.BOLD}{'=' * 56}{C.RESET}")
    print()
    print(f"  {C.DIM}Configure which models can be auto-selected{C.RESET}")
    print(f"  {C.DIM}by the router, and which require manual selection.{C.RESET}")
    print()
...
def _save_config(config_path: Path, config: dict) -> None:
    ...
    print(f"\n{C.GREEN}Config saved to: {config_path}{C.RESET}")
...
def setup_wizard(config_path: Optional[str] = None) -> None:
    ...
    if config_path:
        ...
        raw = _input_colored(
            f"Config file path [{default_path}]",
            C.CYAN,
        )
    ...
    if not models:
        print(f"\n{C.YELLOW}No models found in config.{C.RESET}")
        print(f"{C.DIM}Add models first, then run setup again.{C.RESET}")
        return
    ...
    for key, cfg in models.items():
        ...
        mode_icon = f"{C.GREEN}auto{C.RESET}" if current_mode == "auto" else f"{C.RED}manual{C.RESET}"
        label = f"{name} [{model_id}] — {mode_icon}"
        if cost_hint:
        ...
        model_options.append({
            "id": key,
            "label": label,
            "hint": f"current: {current_mode}",
        })

    # Step 3: Ask which models should be manual-only
    selected_manual = _select_from_list(
        "Which models should be MANUAL-ONLY?",
        model_options,
        multi=True,
    )
    print(f"\n{C.DIM}Selected {len(selected_manual)} model(s) as manual-only{C.RESET}")
    ...
    if expensive:
        print(f"\n{C.YELLOW}Auto-detected expensive models:{C.RESET}")
        for m in expensive:
            print(f"  - {m}")
        if _confirm("Mark these as manual-only too?", default=False):
    ...
    for key, cfg in models.items():
        ...
        icon = f"{C.GREEN}[auto]{C.RESET}" if mode == "auto" else f"{C.RED}[manual]{C.RESET}"
        print(f"  {icon} {key}")
    print(f"{'─' * 50}")
    ...
    if _confirm("Save this configuration?"):
        ...
        print(f"\n{C.DIM}Start the router:  model-router serve{C.RESET}")
    else:
        print(f"\n{C.YELLOW}Cancelled. No changes saved.{C.RESET}")
...
def setup_quick(config_path: Optional[str] = None) -> None:
    ...
    if not models:
        print(f"{C.YELLOW}No models found. Add models to config.yaml first.{C.RESET}")
        return
    ...
    for key in models:
        if key in expensive:
            models[key]["selection_mode"] = "manual"
            print(f"  {C.RED}[manual]{C.RESET} {key} (auto-detected as expensive)")
        else:
            models[key].setdefault("selection_mode", "auto")
            print(f"  {C.GREEN}[auto]{C.RESET}   {key}")

    config["models"] = models
    _save_config(path, config)
    print(f"\n{C.GREEN}Quick setup complete!{C.RESET}")
...
def setup_list(config_path: Optional[str] = None) -> None:
    ...
    print(f"  {C.GREEN}{auto_count} auto{C.RESET} | {C.RED}{manual_count} manual{C.RESET}")
    print()
